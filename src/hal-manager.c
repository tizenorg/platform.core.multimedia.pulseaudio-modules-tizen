...
pa_hal_manager* pa_hal_manager_get(pa_core *core) {
    ...
    if (h->dl_handle) {
        ...
        h->intf.update_stream_connection = dlsym(h->dl_handle, "audio_update_stream_connection");
...
int32_t pa_hal_manager_update_stream_connection(pa_hal_manager *h, hal_stream_connection_info *info) {
    int32_t ret = 0;
    audio_return_t hal_ret = AUDIO_RET_OK;
    ...
    if (AUDIO_IS_ERROR((hal_ret = h->intf.update_stream_connection(h->ah_handle, &hal_info, (uint32_t)info->is_connected)))) {
        pa_log_error("update_stream_connection returns error:0x%x", hal_ret);
        ret = -1;
    }
    return ret;
}
...
int32_t pa_hal_manager_set_message_callback(pa_hal_manager *h, hal_message_callback callback, void *user_data) {
    int32_t ret = 0;
    audio_return_t hal_ret = AUDIO_RET_OK;
...
int32_t pa_hal_manager_unset_message_callback(pa_hal_manager *h, hal_message_callback callback) {
    int32_t ret = 0;
    audio_return_t hal_ret = AUDIO_RET_OK;
