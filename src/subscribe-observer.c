...
static int convert_event_name_to_type(const char *event_name, pa_tizen_event_t *event_type) {
    ...
    if (!strcmp(event_name, "DeviceConnected")) {
        *event_type = PA_TIZEN_EVENT_TYPE_DEVICE_CONNECTION_CHANGED;
        return 0;
    }
...
static int convert_event_type_to_name(pa_tizen_event_t event_type, const char **event_name) {
    if (event_type < 0 || event_type >= PA_TIZEN_EVENT_TYPE_MAX)
        return -1;

    if (event_type == PA_TIZEN_EVENT_TYPE_DEVICE_CONNECTION_CHANGED) {
...
static int get_sender_pid(DBusConnection *c, DBusMessage *got_msg, unsigned *_sender_pid) {
...
static void register_subscriber(pa_hashmap *subscribers, pa_tizen_event_t event_type, unsigned subscriber_pid, unsigned subscription_id) {
...
static void unregister_subscriber(pa_hashmap *subscribers, pa_tizen_event_t event_type, unsigned subscriber_pid, unsigned subscription_id) {
    pa_idxset *event_subscribers;
    const char *event_name;
...
static void register_event_handling(pa_hashmap *handled_events, pa_tizen_event_t event_type, uint32_t event_id, unsigned subscriber_pid, unsigned subscription_id) {
...
/* If 'subscription_id' is less than 0, remove all subscribers which of pid is 'subscriber_pid' */
static void unregister_event_handling(pa_hashmap* handled_events, pa_tizen_event_t event_type, uint32_t event_id, unsigned subscriber_pid, unsigned subscription_id) {
    pa_idxset *handled_subscribers;
    struct subscriber *subscriber_item;
    uint32_t subscriber_idx;
...
static void handle_client_subscribed(pa_subscribe_observer *ob, pa_tizen_event_t event_type, uint32_t sender_pid, unsigned subs_id, bool is_subscribe) {
    const char *event_name;
...
static void handle_client_emergent_exit(pa_subscribe_observer *ob, int client_pid) {
    ...
    for (event_type_idx = 0; event_type_idx < PA_TIZEN_EVENT_TYPE_MAX; event_type_idx++) {
...
static void handle_subscriber_handled_event(pa_subscribe_observer *ob, pa_tizen_event_t event_type, uint32_t event_id,  unsigned sender_pid, unsigned subs_id, DBusMessageIter *var_i) {
    const char *event_name;
...
static pa_hook_result_t device_connection_changed_hook_cb(pa_core *c, pa_device_manager_hook_data_for_conn_changed *data, pa_subscribe_observer *ob) {
    ...
    new_event->event_type = PA_TIZEN_EVENT_TYPE_DEVICE_CONNECTION_CHANGED;
