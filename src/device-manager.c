    ...
    if (pa_sink_isinstance(o)) {
        ...
        if (s->use_internal_codec) {
            if (state == PA_SINK_SUSPENDED) {
                PA_IDXSET_FOREACH(_device_item, dm->device_list, idx) {
                    use_internal_codec = pa_device_manager_use_internal_codec(_device_item, DM_DEVICE_DIRECTION_OUT, DEVICE_ROLE_NORMAL);
        ...
        if (s->use_internal_codec) {
            if (state == PA_SOURCE_SUSPENDED) {
                PA_IDXSET_FOREACH(_device_item, dm->device_list, idx) {
                    use_internal_codec = pa_device_manager_use_internal_codec(_device_item, DM_DEVICE_DIRECTION_IN, DEVICE_ROLE_NORMAL);
                    if (use_internal_codec)
...
pa_bool_t pa_device_manager_use_internal_codec(dm_device *device_item, dm_device_direction_t direction, const char *role) {
    pa_sink *sink;
    pa_source *source;
