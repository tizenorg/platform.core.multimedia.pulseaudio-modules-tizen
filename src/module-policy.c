...
struct userdata {
    ...
    struct {
        ...
        pa_hook_slot *comm_hook_name_value_pair_msg_slot;
    } communicator;
...
static pa_hook_result_t nv_pair_msg_hook_cb(pa_core *c, pa_stream_manager_hook_data_for_nv_pair_msg *data, struct userdata *u) {
    pa_assert(c);
    pa_assert(data);
    pa_assert(u);

    if (pa_streq(data->stream_role, STREAM_ROLE_LOOPBACK)) {
        pa_log_info("[NV_PAIR_MSG] nv_pair_msg_hook_cb is called. stream_role(%s) [name(%s)/value(%d)]",
...
int pa__init(pa_module *m)
{
    ...
    if ((u->communicator.comm = pa_communicator_get(u->core))) {
        ...
        u->communicator.comm_hook_name_value_pair_msg_slot = pa_hook_connect(
                pa_communicator_hook(u->communicator.comm, PA_COMMUNICATOR_HOOK_NAME_VALUE_PAIR_MSG),
                PA_HOOK_EARLY, (pa_hook_cb_t)nv_pair_msg_hook_cb, u);
    }
...
void pa__done(pa_module *m)
{
    ...
    if (u->communicator.comm) {
        ...
        if (u->communicator.comm_hook_name_value_pair_msg_slot)
            pa_hook_slot_free(u->communicator.comm_hook_name_value_pair_msg_slot);
