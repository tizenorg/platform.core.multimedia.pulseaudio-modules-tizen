    ...
    if (command == PROCESS_COMMAND_CHANGE_ROUTE_BY_STREAM_STARTED_WITH_NEW_DATA ||
        ...
        if (cur_max_stream == mine || command == PROCESS_COMMAND_CHANGE_ROUTE_BY_STREAM_FOCUS_CHANGED) {
            ...
            PA_IDXSET_FOREACH(i, streams, idx) {
                ...
                pa_log_debug("role(%s)/priority(%p)/focus_status(0x%x)/stream(%p)", _role, priority, focus_status_int, i);
...
static pa_hook_result_t device_connection_changed_hook_cb(pa_core *c, pa_device_manager_hook_data_for_conn_changed *conn, pa_stream_manager *m) {
    ...
    pa_log_info("[SM] device_connection_changed_hook_cb is called. conn(%p), is_connected(%d), device(%p, %s), direction(0x%x), use_internal_codec(%d)",
...
#if 0
    pa_log_info("[SM] device_information_changed_hook_cb is called. info(%p), changed_info(%d), device(%p)",
...
static pa_hook_result_t need_update_route_hook_cb(pa_core *c, pa_stream_manager_hook_data_for_update_route *info, pa_stream_manager *m) {
#if 1
    pa_log_info("[SM] need_update_route_hook_cb is called. stream_type(%d)", info, info->stream_type);
#endif
    do_notify(m, NOTIFY_COMMAND_CHANGE_ROUTE_START, info->stream_type, FALSE, (info->stream_type==STREAM_SINK_INPUT)?m->cur_highest_priority.sink_input:m->cur_highest_priority.source_output);
...
void pa_stream_manager_is_current_hightest_priority(void *stream, stream_type_t stream_type, pa_bool_t *highest_priority, pa_stream_manager *m) {
