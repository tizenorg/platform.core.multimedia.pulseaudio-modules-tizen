    ...
    if (sp) {
        ...
        if (sp->idx_route_in_devices) {
            ...
            PA_IDXSET_FOREACH(stream, sp->idx_source_outputs, idx) {
                /* find role of stream */
                role = pa_proplist_gets(GET_STREAM_PROPLIST(stream, STREAM_SOURCE_OUTPUT), PA_PROP_MEDIA_ROLE);
                if (role) {
                    pa_log_debug(" -- the role of source_output that belongs to this parent id[%u] is [%s]", id, role);
                    break;
                }
            }
        ...
        if (sp->idx_route_out_devices) {
            ...
            PA_IDXSET_FOREACH(stream, sp->idx_sink_inputs, idx) {
                /* find role of stream */
                role = pa_proplist_gets(GET_STREAM_PROPLIST(stream, STREAM_SINK_INPUT), PA_PROP_MEDIA_ROLE);
                if (role) {
                    pa_log_debug(" -- the role of sink_input that belongs to this parent id[%u] is [%s]", id, role);
                    break;
                }
            }
            /* if any stream that belongs to this id has been activated, do notify right away */
...
static pa_bool_t check_role_to_skip(pa_stream_manager *m, process_command_type_t command, const char *role) {
    ...
    if (m->stream_infos) {
        ...
        if (s) {
            if (IS_ROLE_FOR_EXTERNAL_DEV(role) &&
                (command > PROCESS_COMMAND_PREPARE && command < PROCESS_COMMAND_UPDATE_VOLUME))
                ret = TRUE; /* this role is for external device, set it to skip */
        } else
            ret = TRUE;
    }
...
static pa_hook_result_t device_connection_changed_hook_cb(pa_core *c, pa_device_manager_hook_data_for_conn_changed *conn, pa_stream_manager *m) {
    ...
    pa_log_info("device_connection_changed_hook_cb is called. conn(%p), is_connected(%d), device(%p), direction(0x%x), use_internal_codec(%d)",
            conn, conn->is_connected, conn->device, device_direction, use_internal_codec);
...
void pa_stream_manager_is_available_device_for_role(const char *cur_device_type, const char *new_device_type, const char *role, stream_type_t stream_type, pa_bool_t *available, pa_stream_manager *m) {
    stream_info *si = NULL;
...
void pa_stream_manager_find_next_priority_device_for_role(const char *cur_device_type, const char *role, stream_type_t stream_type, char **next_device_type, pa_stream_manager *m) {
    stream_info *si = NULL;
    pa_idxset *devices = NULL;
    uint32_t idx = 0;
    char *device_type = NULL;
