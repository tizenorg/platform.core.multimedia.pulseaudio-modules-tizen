...
typedef enum _update_type {
    UPDATE_TYPE_FOR_NORMAL_ROLE,
    UPDATE_TYPE_FOR_EXTERNAL_ROLE,
} update_type_t;
...
static pa_hook_result_t select_proper_sink_or_source_hook_cb(pa_core *c, pa_stream_manager_hook_data_for_select *data, struct userdata *u) {
    ...
    if ((data->route_type == STREAM_ROUTE_TYPE_AUTO || data->route_type == STREAM_ROUTE_TYPE_AUTO_ALL) && data->idx_avail_devices) {
        ...
        PA_IDXSET_FOREACH(device_type, data->idx_avail_devices, idx) {
            pa_log_debug("[SELECT][AUTO(_ALL)] avail_device[%u] for this role[%s]: type(%s)", idx, data->stream_role, device_type);
            PA_IDXSET_FOREACH(device, conn_devices, conn_idx) {
                ...
                pa_log_debug("  -- conn_devices, type[%s], subtype[%s], direction[0x%x]", dm_device_type, dm_device_subtype, device_direction);
                if (pa_streq(device_type, dm_device_type) &&
                    ...
                    pa_log_debug("  -- found a matched device: type[%s], direction[0x%x]", device_type, device_direction);
        ...
        if (IS_ROLE_FOR_EXTERNAL_DEV(data->stream_role)) {
            PA_IDXSET_FOREACH(device_type, data->idx_avail_devices, idx) {
                pa_log_debug("[SELECT][MANUAL][ext] avail_device[%u] for this role[%s]: type(%s)", idx, data->stream_role, device_type);
                PA_IDXSET_FOREACH(device_id, data->idx_manual_devices, m_idx) {
                    ...
                    if (device) {
                        ...
                        dm_device_subtype = pa_device_manager_get_device_subtype(device);
                        device_direction = pa_device_manager_get_device_direction(device);
                        pa_log_debug("  -- manual_devices, type[%s], subtype[%s], direction[0x%x], device id[%u]",
            ...
            PA_IDXSET_FOREACH(device_type, data->idx_avail_devices, idx) {
                pa_log_debug("[SELECT][MANUAL] avail_device[%u] for this role[%s]: type(%s)", idx, data->stream_role, device_type);
                PA_IDXSET_FOREACH(device_id, data->idx_manual_devices, m_idx) {
                    device = pa_device_manager_get_device_by_id(u->device_manager, *device_id);
                    if (device) {
                        dm_device_type = pa_device_manager_get_device_type(device);
...
static void _set_device_state(dm_device *device, stream_type_t stream_type, dm_device_state_t device_state) {
    ...
    pa_device_manager_use_internal_codec(device, &use_internal_codec);
...
static pa_hook_result_t route_change_hook_cb(pa_core *c, pa_stream_manager_hook_data_for_route *data, struct userdata *u) {
    ...
    if (pa_streq(data->stream_role, "reset")) {
        ...
        conn_devices = pa_device_manager_get_device_list(u->device_manager);
        PA_IDXSET_FOREACH(device_type, data->idx_avail_devices, idx) {
            pa_log_debug("[ROUTE][AUTO(_ALL)] avail_device[%u] for this role[%s]: type[%s]", idx, route_info.role, device_type);
            PA_IDXSET_FOREACH(device, conn_devices, conn_idx) {
                dm_device_type = pa_device_manager_get_device_type(device);
                dm_device_subtype = pa_device_manager_get_device_subtype(device);
                device_direction = pa_device_manager_get_device_direction(device);
                device_idx = pa_device_manager_get_device_id(device);
                pa_log_debug("  -- conn_devices, type[%s], subtype[%s], direction[0x%x], id[%u]",
                ...
                if (pa_streq(device_type, dm_device_type) &&
                    ...
                    pa_log_debug("  -- found a matched device and set state to ACTIVATED: type[%s], direction[0x%x], id[%u]",
            ...
            if (data->route_type == STREAM_ROUTE_TYPE_AUTO && device) {
                ...
                /* update activated device if it has the AUTO route type and passes though the internal codec */
                pa_device_manager_use_internal_codec(device, &use_internal_codec);
                if (use_internal_codec) {
                    if (!data->origins_from_new_data) {
                        pa_proplist_sets(GET_STREAM_PROPLIST(data->stream, data->stream_type), PA_PROP_MEDIA_ROUTE_AUTO_ACTIVE_DEV, dm_device_type);
                        PA_IDXSET_FOREACH (s, (data->stream_type==STREAM_SINK_INPUT)?sink->inputs:source->outputs, s_idx) {
                            pa_proplist_sets(GET_STREAM_PROPLIST(s, data->stream_type), PA_PROP_MEDIA_ROUTE_AUTO_ACTIVE_DEV, dm_device_type);
                        }
                    }
                }

                if (data->idx_streams) { // need to modify logic to pick streams that is available for this device
...
#if 1 // need to modify logic to pick streams that is available for this device
                else {
                    /* Move sink-inputs/source-outputs if needed */
                    if (data->idx_streams) {
                        PA_IDXSET_FOREACH (s, data->idx_streams, s_idx) {
                            if (sink && (sink != ((pa_sink_input*)s)->sink)) {
                                ...
                                pa_source_output_move_to(s, source, FALSE);
                                pa_log_debug("[ROUTE][AUTO_ALL] *** source-output(%p,%u) moves to source(%p,%s)", s, ((pa_source_output*)s)->index, source, source->name);
                            }
                        }
                    }
                }
        ...
        PA_IDXSET_FOREACH(device_type, data->idx_avail_devices, idx) {
            pa_log_debug("[ROUTE][MANUAL] avail_device[%u] for this role[%s]: type(%s)", idx, data->stream_role, device_type);
            PA_IDXSET_FOREACH(device_id, data->idx_manual_devices, d_idx) {
                pa_log_debug("  -- manual_device[%u] for this role[%s]: device_id(%u)", idx, data->stream_role, *device_id);
                device = pa_device_manager_get_device_by_id(u->device_manager, *device_id);
                if (device) {
                    dm_device_type = pa_device_manager_get_device_type(device);
                    dm_device_subtype = pa_device_manager_get_device_subtype(device);
                    device_direction = pa_device_manager_get_device_direction(device);
                    pa_log_debug("  -- manual_device, type[%s], subtype[%s], direction[0x%x]", dm_device_type, dm_device_subtype, device_direction);
                    if (pa_streq(device_type, dm_device_type) &&
                        (((data->stream_type==STREAM_SINK_INPUT) && (device_direction & DM_DEVICE_DIRECTION_OUT)) ||
                        ((data->stream_type==STREAM_SOURCE_OUTPUT) && (device_direction & DM_DEVICE_DIRECTION_IN)))) {
                        pa_log_debug("  -- found a matched device: type[%s], direction[0x%x]", device_type, device_direction);
                        ...
                        route_info.device_infos[route_info.num_of_devices-1].type = dm_device_type;
                        route_info.device_infos[route_info.num_of_devices-1].direction = (data->stream_type==STREAM_SINK_INPUT)?DIRECTION_OUT:DIRECTION_IN;
                        pa_log_debug("  -- found a matched device and set state to ACTIVATED: type[%s], direction[0x%x]",
...
static void update_sink_or_source_as_device_connection_change(update_type_t update_type, stream_type_t stream_type, pa_idxset *streams, dm_device *device, pa_bool_t is_connected, struct userdata *u) {
    void *s = NULL;
    uint32_t s_idx = 0;
    const char *role = NULL;
    char *route_type_str = NULL;
    const char *device_type = NULL;
    ...
    null_source = (pa_source*)pa_namereg_get(u->core, SOURCE_NULL, PA_NAMEREG_SOURCE);
    if (!null_sink || !null_source) {
        pa_log_error("[UPDATE_SS] could not get null_sink(%p) or null_source(%p)", null_sink, null_source);
        return;
    }

    if (update_type == UPDATE_TYPE_FOR_NORMAL_ROLE) {
        device_type = pa_device_manager_get_device_type(device);
        pa_log_debug("[UPDATE_SS][NR] deivce_type(%s), is_connected(%d))", device_type, is_connected);
        ...
        PA_IDXSET_FOREACH (s, streams, s_idx) { /* streams: core->source_outputs/core->sink_inputs */
            ...
            if (role && !IS_ROLE_FOR_EXTERNAL_DEV(role)) {
                route_type_str = pa_proplist_gets(GET_STREAM_PROPLIST(s, stream_type), PA_PROP_MEDIA_ROLE_ROUTE_TYPE);
                pa_log_debug("[UPDATE_SS][NR] idx(%u), route_type(%s), role(%s)", s_idx, route_type_str, role);
                if (route_type_str && !pa_atoi(route_type_str, (int32_t*)&route_type)) {
                    if (route_type == STREAM_ROUTE_TYPE_AUTO) {
                        if (is_connected) {
                            ...
                            pa_log_debug("  -- sink(%p), s->sink(%p)", sink, ((pa_sink_input*)s)->sink);
                            pa_log_debug("  -- source(%p), s->source(%p)", source, ((pa_source_output*)s)->source);
                            if (sink && (sink != ((pa_sink_input*)s)->sink)) {
                                cur_device_type = pa_proplist_gets(GET_STREAM_PROPLIST(s, stream_type), PA_PROP_MEDIA_ROUTE_AUTO_ACTIVE_DEV);
                                if (cur_device_type) {
                                    pa_stream_manager_is_available_device_for_role(cur_device_type, device_type, role, stream_type, &available, u->stream_manager);
                                    if (available) {
                                        pa_sink_input_move_to(s, sink, FALSE);
                                        pa_log_debug("[UPDATE_SS][AUTO][CONNECTED] *** sink-input(%p,%u) moves to sink(%p,%s), new device(%s)",
                                        ...
                                        _device = pa_device_manager_get_device(u->device_manager, device_type);
                                        if (_device)
                                ...
                                if (cur_device_type) {
                                    ...
                                    if (available) {
                                        pa_source_output_move_to(s, source, FALSE);
                                        pa_log_debug("[UPDATE_SS][AUTO][CONNECTED] *** source-output(%p,%u) moves to source(%p,%s), new device(%s)",
                            ...
                            if (sink && (sink == ((pa_sink_input*)s)->sink)) {
                                ...
                                do {
                                    pa_stream_manager_find_next_priority_device_for_role(device_type, role, stream_type, &next_device_type, u->stream_manager);
                                    pa_log_debug("[UPDATE_SS][AUTO][DISCONNECTED] prev_device(%s), new_device(%s)", device_type, next_device_type);
                                    if (next_device_type) {
                                        next_device = pa_device_manager_get_device(u->device_manager, next_device_type);
                                        if (next_device) {
                                            next_sink = pa_device_manager_get_sink(next_device, DEVICE_ROLE_NORMAL);
                                            if (next_sink) {
                                                pa_sink_input_move_to(s, next_sink, FALSE);
                                                pa_log_debug("[UPDATE_SS][AUTO][DISCONNECTED]  *** sink-input(%p,%u) moves to sink(%p,%s), new device(%s)",
                                ...
                                if (!next_device_type || !next_sink) {
                                    pa_sink_input_move_to(s, null_sink, FALSE);
                                    pa_log_debug("[UPDATE_SS][AUTO][DISCONNECTED]  *** sink-input(%p,%u) moves to sink(%p,%s)",
                                ...
                                do {
                                    ...
                                    if (next_device_type) {
                                        next_device = pa_device_manager_get_device(u->device_manager, next_device_type);
                                        if (next_device) {
                                            next_source = pa_device_manager_get_source(next_device, DEVICE_ROLE_NORMAL);
                                            if (next_source) {
                                                pa_source_output_move_to(s, next_source, FALSE);
                                                pa_log_debug("[UPDATE_SS][AUTO][DISCONNECTED]  *** source-output(%p,%u) moves to source(%p,%s), new device(%s)",
                                ...
                                if (!next_device_type || !next_source) {
                                    ...
                                    pa_log_debug("[UPDATE_SS][AUTO][DISCONNECTED]  *** source-output(%p,%u) moves to source(%p,%s)",
        ...
        if (cnt) {
            PA_IDXSET_FOREACH (s, streams, s_idx) { /* streams: core->source_outputs/core->sink_inputs */
                ...
                if (cur_device_type) {
                    for (cnt = 0; cached_prev_dev_list[cnt].device_type; cnt++) {
                        pa_log_warn("[UPDATE_SS][STATE_CHANGE_DEACTIVATED] cur_device(%s), cached_device(%s)", cur_device_type, cached_prev_dev_list[cnt].device_type);
            ...
            if (stream_type == STREAM_SINK_INPUT) {
                ...
                if (combine_sink && pa_idxset_size(combine_sink->inputs)) {
                    pa_log_warn("[UPDATE_SS][STATE_CHANGE_DEACTIVATED] combine sink has streams");
                    return;
                }
            }
            for (cnt = 0; cached_prev_dev_list[cnt].device_type; cnt++) {
                if (cached_prev_dev_list[cnt].count == 0) {
                    pa_log_debug("[UPDATE_SS][STATE_CHANGE_DEACTIVATED] device_type(%s)", cached_prev_dev_list[cnt].device_type);
                    ...
                    if (_device)
                        ...
                        pa_log_warn("[UPDATE_SS][STATE_CHANGE_DEACTIVATED] _device is null");
                }
            }
        }

    } else if (update_type == UPDATE_TYPE_FOR_EXTERNAL_ROLE) {
        if (!is_connected) {
            PA_IDXSET_FOREACH (s, streams, s_idx) { /* streams: source->outputs/sink->inputs */
                ...
                if (role && IS_ROLE_FOR_EXTERNAL_DEV(role)) {
                    if (stream_type == STREAM_SOURCE_OUTPUT) {
                        ...
                        pa_log_debug("[UPDATE_SS][EXT][DISCONNECTED] *** source-output(%p,%u) moves to source(%p,%s)", s, ((pa_source_output*)s)->index, null_source, null_source->name);
                    } else {
                        /* move it to null sink if this role is for external device */
                        pa_sink_input_move_to((pa_sink_input*)s, null_sink, FALSE);
                        pa_log_debug("[UPDATE_SS][EXT][DISCONNECTED] *** sink-input(%p,%u) moves to sink(%p,%s)", s, ((pa_sink_input*)s)->index, null_sink, null_sink->name);
                    }
                }
            }
        } else {
...
static pa_hook_result_t device_connection_changed_hook_cb(pa_core *c, pa_device_manager_hook_data_for_conn_changed *conn, struct userdata *u) {
    ...
    pa_device_manager_use_internal_codec(conn->device, &use_internal_codec);
    if (!use_internal_codec) {
                ...
                if (!combine_sink) {
                    ...
                    if (combine_sink->inputs) {
                        PA_IDXSET_FOREACH (s, combine_sink->inputs, s_idx) {
                            ...
                            /* TODO: need to re-route this stream to the remaining device */
                        }
                    }
                    pa_sink_suspend(combine_sink, TRUE, PA_SUSPEND_USER);
                    pa_module_unload(u->core, u->module_combine_sink, TRUE);
        ...
        if (device_direction & DM_DEVICE_DIRECTION_IN) {
            ...
            if (!source)
                ...
                update_sink_or_source_as_device_connection_change(UPDATE_TYPE_FOR_EXTERNAL_ROLE, STREAM_SOURCE_OUTPUT, source->outputs, conn->device, conn->is_connected, u);

            update_sink_or_source_as_device_connection_change(UPDATE_TYPE_FOR_NORMAL_ROLE, STREAM_SOURCE_OUTPUT, u->core->source_outputs, conn->device, conn->is_connected, u);
        }
        if (device_direction & DM_DEVICE_DIRECTION_OUT) {
            ...
            if (!sink)
                ...
                update_sink_or_source_as_device_connection_change(UPDATE_TYPE_FOR_EXTERNAL_ROLE, STREAM_SINK_INPUT, sink->inputs, conn->device, conn->is_connected, u);

            update_sink_or_source_as_device_connection_change(UPDATE_TYPE_FOR_NORMAL_ROLE, STREAM_SINK_INPUT, u->core->sink_inputs, conn->device, conn->is_connected, u);
        }
    }
