...
int32_t pa_hal_manager_get_buffer_attribute(pa_hal_manager *h, hal_stream_info *info, uint32_t *maxlength, uint32_t *tlength, uint32_t *prebuf, uint32_t* minreq, uint32_t *fragsize) {
    ...
    pa_log_info("latency:%s, rate:%u, format:%d, channels:%u", info->latency, info->sample_spec->rate, info->sample_spec->format, info->sample_spec->channels);

    if (AUDIO_IS_ERROR(hal_ret = h->intf.get_buffer_attr(h->data, info->direction, info->latency, info->sample_spec->rate, info->sample_spec->format, info->sample_spec->channels, maxlength, tlength, prebuf, minreq, fragsize))) {
...
int32_t pa_hal_manager_pcm_avail (pa_hal_manager *h, pcm_handle pcm_h) {
    ...
    if (AUDIO_IS_ERROR(hal_ret = h->intf.pcm_avail(pcm_h))) {
...
int32_t pa_hal_manager_pcm_write (pa_hal_manager *h, pcm_handle pcm_h, const void *buffer, uint32_t frames) {
    ...
    if (AUDIO_IS_ERROR(hal_ret = h->intf.pcm_write(pcm_h, buffer, frames))) {
