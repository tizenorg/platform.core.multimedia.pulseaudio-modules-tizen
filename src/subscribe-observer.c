...
static int convert_event_name_to_type(const char *event_name, pa_event_t *event_type) {
    if (!event_name || !event_type)
        return -1;

    if (!strcmp(event_name, "DeviceConnected")) {
        *event_type = PA_EVENT_TYPE_DEVICE_CONNECTION_CHANGED;
        return 0;
    }
...
static int convert_event_type_to_name(pa_event_t event_type, const char **event_name) {
    if (event_type < 0 || event_type >= PA_EVENT_TYPE_MAX)
        return -1;

    if (event_type == PA_EVENT_TYPE_DEVICE_CONNECTION_CHANGED) {
...
static int subscriber_compare_func(const void *a, const void *b) {
    ...
    unsigned pid_a, pid_b, sid_a, sid_b;
...
static int get_sender_pid(DBusConnection *c, DBusMessage *got_msg, unsigned int *_sender_pid) {
...
static void register_subscriber(pa_hashmap *subscribers, pa_event_t event_type, unsigned subscriber_pid, unsigned subscription_id) {
    ...
    if(!event_subscribers) {
        event_subscribers = pa_idxset_new(pa_idxset_trivial_hash_func, subscriber_compare_func);
...
static void unregister_subscriber(pa_hashmap *subscribers, pa_event_t event_type, unsigned subscriber_pid, unsigned subscription_id) {
    pa_idxset *event_subscribers;
    struct subscriber subscriber_tmp = {subscriber_pid, subscription_id};
    ...
    if (subscription_id > 0)
        pa_idxset_remove_by_data(event_subscribers, &subscriber_tmp, NULL);
    else
        PA_IDXSET_FOREACH(subscriber_item, event_subscribers, subscriber_idx) {
            if (subscriber_item->pid == subscriber_pid) {
                pa_idxset_remove_by_index(event_subscribers, subscriber_idx);
            }
        }
}

static void register_event_handling(pa_hashmap *handled_events, pa_event_t event_type, unsigned event_id, unsigned subscriber_pid, unsigned subscription_id) {
...
/* If 'subscription_id' is less than 0, remove all subscribers which of pid is 'subscriber_pid' */
static void unregister_event_handling(pa_hashmap* handled_events, pa_event_t event_type, unsigned event_id, unsigned subscriber_pid, unsigned subscription_id) {
    pa_idxset *handled_subscribers;
    struct subscriber *subscriber_item;
    uint32_t subscriber_idx;
...
static void handle_client_subscribed(pa_subscribe_observer *ob, pa_event_t event_type, unsigned sender_pid, unsigned subs_id, pa_bool_t is_subscribe) {
...
static void handle_client_emergent_exit(pa_subscribe_observer *ob, int client_pid) {
    ...
    for (event_type_idx = 0; event_type_idx < PA_EVENT_TYPE_MAX; event_type_idx++)
...
static pa_bool_t is_event_handled_all(pa_subscribe_observer *ob, pa_event_t event_type, unsigned event_id) {
    ...
    if(!(subscribers = pa_hashmap_get(ob->subscribers, PA_INT_TO_PTR(event_type)))) {
        pa_log_error("No subscribers for event : %s", event_name);
        return FALSE;
    }
    if(!(handled_subscribers = pa_hashmap_get(ob->handled_events, &event))) {
        pa_log_error("No handled_subscribers for event : %s, id : %u", event_name, event_id);
        return FALSE;
    }

    subs_cnt = pa_idxset_size(subscribers);
    handled_subs_cnt = pa_idxset_size(handled_subscribers);

    pa_log_debug("[DEBUG] subscriber count : %u, handled subscriber count : %u", subs_cnt, handled_subs_cnt);
...
static void handle_all_subscribers_handled_event(pa_subscribe_observer *ob, pa_event_t event_type, unsigned event_id) {
...
static void handle_subscriber_handled_event(pa_subscribe_observer *ob, pa_event_t event_type, unsigned event_id,  unsigned sender_pid, unsigned subs_id, DBusMessageIter *var_i) {
    const char *event_name;
...
static DBusHandlerResult dbus_filter_audio_client_handler(DBusConnection *c, DBusMessage *m, void *userdata) {
    ...
    if (dbus_message_is_signal(m, INTERFACE_AUDIO_CLIENT, "ClientSubscribed")) {
        ...
        if (!dbus_message_get_args(m, NULL, DBUS_TYPE_STRING, &event_name,
                                            ...
                                            DBUS_TYPE_BOOLEAN, &is_subscribe,
                                            DBUS_TYPE_INVALID)) {
            goto fail;
        } else {
...
static pa_hook_result_t device_connection_changed_hook_cb(pa_core *c, pa_device_manager_hook_data_for_conn_changed *data, pa_subscribe_observer *ob) {
    ...
    new_event->event_type = PA_EVENT_TYPE_DEVICE_CONNECTION_CHANGED;
    new_event->event_id = data->event_id;

    /* We assume that we got connection changed hook earlier than 'ClientSignalHandled' signal */
    handled_subscribers = pa_idxset_new(pa_idxset_trivial_hash_func, subscriber_compare_func);
...
pa_subscribe_observer* pa_subscribe_observer_get(pa_core *c) {
    ...
    ob->comm.comm_hook_device_connection_changed_slot = pa_hook_connect(pa_communicator_hook(ob->comm.comm, PA_COMMUNICATOR_HOOK_DEVICE_CONNECTION_CHANGED),
            PA_HOOK_EARLY + 10, (pa_hook_cb_t)device_connection_changed_hook_cb, ob);
